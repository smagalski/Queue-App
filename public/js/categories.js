...
import { DEFAULT_CATEGORY_RULES, CAT_TALLY_COLORS, DAYS, MONTHS, P_COLS, P_TEXT, P_NAMES } from './constants.js';
...
export function showConfirmRestoreCategories() { document.getElementById('confirmRestoreCategories').classList.add('visible'); }
export function hideConfirmRestoreCategories() { document.getElementById('confirmRestoreCategories').classList.remove('visible'); }

export function restoreDefaultCategories() {
  hideConfirmRestoreCategories();
  state.categoryRules = JSON.parse(JSON.stringify(DEFAULT_CATEGORY_RULES));
  // Custom categories are gone, so any task pointing at one would otherwise
  // show a dangling internal id instead of a real name.
  const validIds = new Set(state.categoryRules.map(c => c.id));
  for (const t of [...state.tasks, ...state.doneTasks]) {
    if (t.categoryOverride != null && !validIds.has(t.categoryOverride)) delete t.categoryOverride;
  }
  _saveCategoryRules();
  renderCategoryManager();
  renderCategoryTally();
